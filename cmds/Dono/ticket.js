  ...
  async run({ message, args, prefix, author }, t) {
    ...
    if (!args[0])
      return message.reply(
        `${Emojis.Help} - ${message.author}, comandos do sistema:\n\n> **${prefix}ticket msg** - para abrir um ticket\n> **${prefix}ticket fechar** - para fechar um ticket\n> **${prefix}ticket force-close** - para forçar o fechamento de um ticket(adms)\n.`,
      )
    ...
    const DT = await this.client.database.users.findOne({ userId: USER.id })
    ...
    if (['info'].includes(args[0].toLowerCase())) {
      const TICKETS = new ClientEmbed(author)
        ...
        .setDescription(
          `${
            !DT.ticket.have
              ? `Este usuário não possui nenhum ticket em aberto.`
              : `Este membro possui um ticket em aberto, informações:\n\n> Canal: <#${
                  DT.ticket.channel
    ...
    if (['staff'].includes(args[0].toLowerCase())) {
      ...
      if (!ROLE)
        return message.reply(
          `${message.author}, mencione/insira o ID do cargo desejado para setar no sistema de tickets.`,
        )
    ...
    if (['forceclose', 'force-close', 'fc'].includes(args[0].toLowerCase())) {
      try {
        ...
        const DT = await this.client.database.users.findOne({
          userId: TARGET.id,
        })
        ...
        setTimeout(async () => {
          try {
            ...
            if (err)
              message.reply(
                `${message.author}, o canal do ticket já havia sido deletado manualmente, então removi o membro de minha database de Tickets.`,
              )
          }
    ...
    if (['fechar', 'close'].includes(args[0].toLowerCase())) {
      ...
      setTimeout(async () => {
        try {
          ...
          if (err)
            message.reply(
              `${message.author}, o canal do ticket já havia sido deletado manualmente, então removi você de minha database de Tickets.`,
            )
        }
