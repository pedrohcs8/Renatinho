...
module.exports = class Shards extends Command {
  ...
  async run({ message, args, prefix, author }, t) {
    ...
    const uptime = await this.client.shard.broadcastEval(
        (client) => this.uptime,
      ),
      ping = await this.client.shard.broadcastEval((client) =>
        Math.round(this.ws.ping),
      ),
      ram = await this.client.shard.broadcastEval(
        (client) => process.memoryUsage().rss,
      ),
      guilds = await this.client.shard.broadcastEval(
        (client) => this.guilds.cache.size,
      ),
      users = await this.client.shard.broadcastEval(
        (client) => this.users.cache.size,
      )
